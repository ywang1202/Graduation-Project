...
# for num_epoch in range(10):
    ...
    with tf.Session() as sess:
        ...
        for i in range(len(data_ir)):
            ...
            image_path = os.path.join(os.getcwd(), 'outputs')
            if not os.path.exists(image_path):
                os.makedirs(image_path)
            if i<=8:
                image_path = os.path.join(image_path, 'GAN_fuse_0' + str(i+1) + ".png")
            else:
                image_path = os.path.join(image_path, 'GAN_fuse_' + str(i+1) + ".png")
            end=time.time()
            # print(out.shape)
